...
async function AdmincheckAPIKey(apikey) {
  try {
    return await new Promise((resolve, reject) => {
      conn.query(
        `SELECT * FROM admin WHERE apikey = '${apikey}'`,
        (error, results) => {
          if (error) return reject(status.internalservererror());
          if (results.length <= 0) resolve(false);
          resolve(true);
...
function createInstance() {
  conn.query(`update instance set isActive = 0 where 1`, (err, result) => {
    if (err || result.affectedRows <= 0)
      return console.log(status.internalservererror());
...
const CREDENTIALS = JSON.parse(
  fs.readFileSync("spreadsheet-388213-84c50f351ad0.json")
);
    ...
    if (answers.length > 1) {
      ...
      conn.query(`select * from support_agents`, (err, result) => {
        ...
        if (result.length > 0) {
          ...
          const Agent_email =
            agentsInCategory[
              Math.floor(Math.random() * agentsInCategory.length)
            ];
          ...
          conn.query(
            ...
            async (error, results) => {
              ...
              if (results.length > 0) {
                ...
                let count = 1;
                let msg =
                  "*you have to write a number correspond to the problem which displaying in below message for the information.*\n";
          ...
          conn.query(
            ...
            async (error, results) => {
              ...
              if (results.length > 0) {
                ...
                await obj[iid].send_whatsapp_message(
                  message.from,
  ...
  try {
    if (isValidapikey) {
      ...
      await obj[iid].client.on("authenticated", (session) => {
        conn.query(
          `update instance set isActive = 1 where instance_id = '${iid}'`,
          (err, result) => {
  ...
  try {
    if (isValidapikey) {
      ...
      obj[iid].client.on("authenticated", (session) => {
        conn.query(
          `update admin set isActive = 1 where apikey = '${iid}'`,
          (err, result) => {
            if (err || result.affectedRows < 1)
              res.send(status.internalservererror());
...
app.get("/disconnected/:iid", async (req, res) => {
  try {
    ...
    obj[iid]
      ...
      .then(() => {
        conn.query(
          `update instance set isActive = 0 where instance_id = '${iid}'`,
          (err, result) => {
            if (err || result.affectedRows < 1)
              res.send(status.internalservererror());
            res.send(status.ok());
...
app.get("/adminbotdisconnected/:iid", async (req, res) => {
  try {
    ...
    obj[iid]
      ...
      .then(() => {
        conn.query(
          `update admin set isActive = 0 where apikey = '${iid}'`,
          (err, result) => {
            if (err || result.affectedRows < 1)
              res.send(status.internalservererror());
            res.send(status.ok());
        ...
        function (err, result) {
          ...
          for (let i = 0; i < contacts.length; i++) {
            if (obj[iid]) {
              ...
              obj[iid]
                .send_whatsapp_message(chatId, msgarr[i])
                .then((messageId) => {
                  ...
                  conn.query(
                    ...
                    [
                      msgid,
                      msgarr[i],
                      "Bulk Message Template localfile",
                      chatId,
                      iid,
                      apikey,
                      token,
                ...
                .catch((error) => {
                  console.log(
                    `error in Sending schedule Message ::::::: <${error}>`
                  );
...
app.post("/sendimage", async function (req, res) {
  ...
  try {
    if (isValidapikey) {
      ...
      conn.query(
        `select * from instance where instance_id = '${iid}' and apikey = '${apikey}'`,
        function (err, result) {
          if (err || result.length <= 0) return res.send(status.forbidden());
            ...
            uploadedFile.mv(uploadPath, async function (err) {
              ...
              if (obj[iid]) {
                ...
                for (let i = 0; i < phonearray.length; i++) {
                  ...
                  await obj[iid]
                    .send_whatsapp_document(chatId, media, caption)
                    .then((messageId) => {
                      cloudinary.uploader
                        .upload(filepath, { folder: "M3" })
                        .then((data) => {
                          conn.query(
                            `insert into message values(?,?,?,?,?,?,?,?)`,
                            [
                              msgid,
                              data.secure_url,
                              `Document-${req.files.image.mimetype}`,
                              chatId,
                              iid,
                              apikey,
                              token,
                        ...
                        .catch((err) => {
                          console.log(
                            `error in storing Document on cloudinary ::::::: <${err}>`
                          );
                          conn.query(
                            `insert into message values(?,?,?,?,?,?,?,?)`,
                            [
                    ...
                    .catch((error) => {
                      console.error(
                        `error in sending Document ::::::: <${error}>`
                      );
    ...
    if (isValidapikey) {
      ...
      conn.query(
        ...
        async function (err, result) {
          ...
          if (obj[iid]) {
            obj[iid]
              .send_whatsapp_message(chatId, message)
              .then((messageId) => {
          ...
          for (let i = 0; i < contacts.length; i++) {
            ...
            if (obj[iid]) {
              obj[iid]
                ...
                .catch((error) => {
                  console.log(
                    `error in Sending Bulk Message to Channel ::::::: <${error}>`
                  );
                  return res.send(status.userNotValid());
                });
            } else {
                                ...
                                if (i === contacts.length - 1) {
                                  conn.query(
                                    ...
                                    async function (err, result) {
                                      ...
                                      let body =
                                        "Your scheduled task has completed without any error.";
                                      sendEmail(sender, to, subject, body)
                                        .then(() => {
                                          return console.log(
                                            "Email Sent Scuuessfully"
                                          );
                                        })
                                        .catch((error) => {
                                          return console.log(
                                            `error in Sending  E-Mail ::::::: <${error}>`
                                          );
                                        });
                                    }
                                  );
                            ...
                            await obj[iid]
                              ...
                              .then((messageId) => {
                                ...
                                cloudinary.uploader
                                  ...
                                  .catch((err) => {
                                    console.log(
                                      `error in storing Document on cloudnary ::::::: <${err}>`
                                    );
                                    conn.query(
                                      `insert into message values(?,?,?,?,?,?,?,?)`,
                                      [
...
app.get("/refreshtoken/:iid", async (req, res) => {
  ...
  try {
    if (isValidapikey) {
      ...
      let iid = req.params.iid;
      conn.query(
        `UPDATE instance SET token = '${token}' where apikey = '${apikey}' and instance_id = '${iid}'`,
        function (err) {
          if (err) console.log(err);
          res.send(token);
        }
...
app.post("/get-channel-contact", async (req, res) => {
  ...
  try {
    if (isValidapikey) {
      ...
      conn.query(
        `SELECT cc.channel_id,c.contact_id,c.name,ch.channelName,c.phone,c.email FROM contact_channel cc, contact c, channel ch WHERE cc.channel_id = ch.channel_id AND cc.contact_id = c.contact_id and cc.apikey = '${apikey}' AND cc.channel_id = '${channel_id}' AND cc.instance_id = '${req.body.iid}' order by c.name asc`,
        (err, result) => {
          if (err) return res.send(status.internalservererror());
...
app.post("/create_custom_template", async function (req, res) {
  ...
  try {
    if (isValidapikey) {
      ...
      conn.query(
        `insert into cstm_template values(?,?,?,?,?)`,
    ...
    if (isValidapikey) {
      conn.query(
        `SELECT m.instance_id AS iid, i.i_name AS i_name, 
            ...
            FROM email m JOIN instance i ON m.instance_id = i.instance_id GROUP BY m.apikey, m.instance_id, i.i_name having apikey = '${apikey}'`,
        function (err, result) {
          if (err) return res.send(status.internalservererror());
          if (result.length <= 0) return res.send(status.nodatafound());
          res.send(result);
...
app.get("/email_summary_admin", async (req, res) => {
  conn.query(
    `SELECT COUNT(CASE WHEN m.email_type = 'Bulk Mail Template' THEN 1 END) AS bulk_mail_template_count,
            COUNT(CASE WHEN m.email_type = 'Bulk Mail Custom' THEN 1 END) AS bulk_mail_custom_count, 
            COUNT(CASE WHEN m.email_type = 'Bulk Mail Channel' THEN 1 END) AS bulk_mail_channel_count
            FROM email m`,
    function (err, result) {
...
app.get("/get-landingpage-data", (req, res) => {
  try {
    conn.query(
      `SELECT COUNT(*) AS total FROM users UNION ALL SELECT COUNT(*) FROM instance UNION ALL SELECT COUNT(*) FROM message`,
      (err, result) => {
          ...
          if (obj[iid]) {
            obj[iid]
              ...
              .catch((error) => {
                res.send({
                  status_code: "1013",
                  Message: "Error in sending message / Inactive instance",
                });
              });
          } else {
            res.send({
    ...
    if (isValidapikey) {
      tableData(
        ...
        (result) => {
          ...
          let query = `delete from ${req.params.fld} where ${
...
app.put("/updateWorkflowSteps", (req, res) => {
  console.log(req.body);
  conn.query(
    "select * from workflow where workflow_id='" + req.body.workflow_id + "'",
    (err, result) => {
      if (err) return res.send(status.internalservererror());
      if (result.length > 0) {
        let flag = true;
        ...
        conn.query(
          `update workflow set index_no=index_no-1 where workflow_id='${req.body.workflow_id}' and index_no>=${req.body.index_start}`,
          (err, rslt) => {
            if (err) res.send(status.internalservererror());
            console.log(rslt);
...
app.get("/chartemail", (req, res) => {
  ...
  conn.query("SELECT email_type,email_id FROM email", (err, result) => {
    if (err) {
      console.log(err);
    }
    if (result.length > 0) {
      ...
      var obj = {
        templateBulk: template_bulk,
        customBulk: custom_bulk,
        channelBulk: channel_bulk,
      };
      res.send(obj);
    }
  });
        ...
        conn.query(
          ...
          async (err, resp) => {
            ...
            sendEmail(
              sender,
              assignedAgent,
              `New support ticket (${t_id}): ${t_type}`,
...
app.post("/AgentReplyToTicket", async (req, res) => {
  ...
  switch (category) {
    case "whatsapp": {
      supportclient[sindex].client.sendMessage(
        chatId,
...
app.post("/createworkflow", async (req, res) => {
  ...
  try {
    if (isValidapikey) {
      ...
      let wname = req.body.wname;
      conn.query(
        `insert into workflow (workflow_id,workflow_name,apikey,instance_id) values(?,?,?,?)`,
